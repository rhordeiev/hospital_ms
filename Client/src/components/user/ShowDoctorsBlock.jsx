...
} from '../../../store/apis/headOfDepartmentApi';

export default function ShowHeadsOfDepartmentsBlock() {
  const { register } = useForm();
  const [getDoctors] = useLazyGetDoctorsInfoQuery();
  const [changeViewDoctors] = useLazyChangeViewDoctorsInfoQuery();
  const [deleteDoctor] = useDeleteDoctorMutation();
  const [aggregateDoctors] = useLazyAggregateDoctorsInfoQuery();
  ...
  const aggregate = async () => {
    const aggregateResult = await aggregateDoctors({
  ...
  const deleteDoctorClick = async (e) => {
    await deleteDoctor(e.target.id);
    setDoctors(
      doctors.filter((current) => current.id !== parseInt(e.target.id, 10)),
    );
    aggregate();
  };
  ...
  const onChange = async () => {
    if (searchRef.current.value.trim() === '') {
      ...
      const result = await changeViewDoctors({
        value: searchRef.current.value,
        filter: filterRef.current.value,
        sort: sortRef.current.value,
        sortdirection: sortdirectionRef.current.value,
  ...
  async function getParams() {
    const fetchedResult = (await getDoctors()).data;
    ...
    propertiesNames.set('cabinet', 'кабінетом');
  ...
  useEffect(() => {
    ...
    Array.from(elements).forEach((current) => {
      current.addEventListener('click', deleteDoctorClick);
    });
  });
