...
import { useAddHeadOfDepartmentMutation } from '../../../store/apis/headDoctorApi';

export default function AddHeadOfDepartmentBlock() {
  const {
    register,
    handleSubmit,
    formState: { errors },
  } = useForm();
  const [addHeadOfDepatment] = useAddHeadOfDepartmentMutation();

  const onSubmit = async (enteredData) => {
    await addHeadOfDepatment(enteredData);
  };
